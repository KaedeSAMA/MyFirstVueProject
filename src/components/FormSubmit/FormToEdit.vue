...
import MajorAndClass from './MajorAndClass.vue'
import FirstVolunteer from './FirstVolunteer.vue'
import SecondVolunteer from './SecondVolunteer.vue'
import WhetherAdjustment from './WhetherAdjustment.vue'
...
export default {
    ...
    watch: {
        editInnerData () {
            console.log("innerData改变了");
            this.getEditInnerData()
        }
    },
    methods: {
        ...
        getEditInnerData () {
            this.send_form = this.editInnerData
        },
        post_form () {
            ...
            if (this.send_form.stdId === '' || this.send_form.stdName === '' || this.send_form.major === '' || this.send_form.classNum === '' || this.send_form.stdQQ === "" || this.send_form.stdPhone === "" || this.send_form.firstWill.organization === '' || this.send_form.firstWill.branch === "") {
                ...
                if (/\d/.test(this.send_form.stdQQ)) {
                    if ((/^1[3456789]\d{9}$/.test(this.send_form.stdPhone))) {
                        if (this.send_form.isDispensing === '是') {
                            this.send_form.isDispensing = true;
                        }
                        if (this.send_form.isDispensing === '否') {
                            this.send_form.isDispensing = false;
                        }
                        //通过了前端表单检测，开始发请求
